...
from settings import GENERAL_DATA_DIR, EUROPARL_DATA_DIR
...
def maybe_download_and_extract_dataset(dataset_parser, language_code="de", tmx=True):
    # dataset is dict like {'name': 'europarl', 'genre': 'law', 'version': 'v7', 'url': 'http://opus.nlpl.eu/download.php?f=Europarl/v7/tmx'}

    data_dir = os.path.join(GENERAL_DATA_DIR, dataset_parser.dataset_name, language_code)
    ...
    if tmx:
        ...
        raw_url = dataset_parser.get_dataset_url()
        url = os.path.join(raw_url, append_string)
        ...
        try:
            ...
            url = os.path.join(dataset_parser.url, append_string)
            raw_file = download.maybe_download_and_extract(url=url, download_dir=data_dir, language_code=language_code)
            return raw_file
    else:
